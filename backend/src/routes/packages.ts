 ...
 */
// Typed like the routes above: middleware in front of a handler stops Express 5
// narrowing req.params from the path on its own.
packagesRouter.post("/:id/tamper-check", retryLimiter, async (req: Request<{ id: string }>, res) => {
  const pkg = await prisma.package.findUnique({ where: { id: req.params.id } });
  if (!pkg) return res.status(404).json({ error: "package not found" });
